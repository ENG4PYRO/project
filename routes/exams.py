...
def exam_page():
    ...
    if request.method == "POST":
        ...
        conn.execute("INSERT INTO exams (user_id, subject, exam_date, exam_time, reminder_hours) VALUES (?, ?, ?, ?, ?)", 
                     (user_id, request.form.get("subject"), request.form.get("exam_date"), request.form.get("exam_time"), reminder_hours))
        conn.commit()
        conn.close()
        return redirect(url_for("exams.exam_page"))
        
    exam_items = conn.execute("SELECT id, subject, exam_date, exam_time, reminder_hours FROM exams WHERE user_id = ?", (user_id,)).fetchall()
    conn.close()
